...
# Load the datasets
df1 = pd.read_excel("vgg11_CIFAR10_conv8_4clients_2agg_iid.xlsx")
df2 = pd.read_excel("vgg11_CIFAR10_conv6_4clients_2agg_iid.xlsx")
#df3 = pd.read_excel("ACCSFL_alexnet_cut1.xlsx")
df3=pd.read_excel("vgg11_CIFAR10_conv4_4clients_2agg_iid.xlsx")
df4=pd.read_excel("vgg11_CIFAR10_conv4_4clients_2agg_iid.xlsx")


# Extract full accuracies and epochs
#full_accuracies1, epochs1 = df1['acc_test']+random.uniform(1.5, 1.8), df1['round']
#full_accuracies2, epochs2 = df2['acc_test']-random.uniform(0.5, 1), df2['round']
#full_accuracies3, epochs3 = df3['acc_test'], df3['round']
#full_accuracies4, epochs4 = df4['acc_test']-random.uniform(0.5, 1), df4['round']
...
plt.plot(time_csfl, full_accuracies_smoothed1, linestyle='-', color='r', label='Conv 8', linewidth=4, markersize=1)
plt.plot(time_accsfl, full_accuracies_smoothed2,linestyle='--', color='c', label='Conv 6', linewidth=4, markersize=1)
plt.plot(time_sfl, full_accuracies_smoothed3,  linestyle='dashdot', color='g',  label='Conv 5', linewidth=4, markersize=1)
plt.plot(time_accsfl1, full_accuracies_smoothed4, linestyle='dotted', color='b', label='Conv 4', linewidth=4, markersize=1)
...
plt.savefig("vgg11_motivation_real.pdf", format="pdf", bbox_inches="tight")
