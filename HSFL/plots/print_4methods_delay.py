...
df_accsfl = pd.read_excel("alexnet_MNIST_conv5_100clients_100_agg_iid.xlsx")#replace with 80aggregators
...
epochs_accsfl, acc_accsfl = df_accsfl['round']-random.uniform(0.5, 1), df_accsfl['acc_test']
...
# Plotting the lines
plt.plot(time_csfl, acc_csfl, linestyle='--', color='c', label='HSFL-ll', linewidth=4, markersize=1)
plt.plot(time_sfl_tirana, acc_sfl_tirana, linestyle='-', color='g', label='Multihop SFL',  linewidth=4, markersize=1)
plt.plot(time_sfl, acc_sfl, linestyle='-', color='b', label='SFL',  linewidth=4, markersize=1)
plt.plot(time_accsfl, acc_accsfl, linestyle='-', color='m', label='LocSplitFed',  linewidth=4, markersize=1)
