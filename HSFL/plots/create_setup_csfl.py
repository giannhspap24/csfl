def compute_latency_sfl(v,V,layer_mflops_one_sample,layer_mbytes,R,N,samples,p_n,p_s):
    ...
    D2=term1_1+term1_2+term1+term1_2+term1_3
    D3 = D0
    D_round = D0 + D2 + D3
...
def compute_latency_adaptsfl(v,V,layer_mflops_one_sample,layer_mbytes,R,N,samples,p_n,p_k,p_s):
    ...
    D0 = max( sum(layer_mbytes[:v]) / R,  0 )
    #Clients perform FP
    term1_1=(samples * sum(layer_mflops_one_sample[:v]))*10**6 / p_k or p_k
...
def find_optimal_h_v_sfl(V, layer_mflops_one_sample, layer_mbytes, R, N, samples, p_n, p_k, p_s):
    best_v = None
    min_delay = float('inf')
...
if __name__ == "__main__":
    ...
    mylambda=0.3
    #Server parameters
    pn=1.5 * 4 *1#Ghz (ghz per core per 2 Flops/cycle)
    pk=2.8 * 8 * 1#Ghz
    ps=100 #Ghz
    ...
    #sfl_delay=compute_latency_sfl(optimal_v,V,layer_mflops_one_sample,layer_mbytes,R,N,samples,p_n,p_s)
