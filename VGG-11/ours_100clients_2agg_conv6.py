...
cut_layer = "conv6" 
program = "vgg11_CIFAR10_conv6_100clients_2agg_iid"
print(f"---------{program}----------")
start = time.time()
        ...
        for iter in range(self.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.ldr_train):
               ...
               # Forward pass through client model
                ...
                #Added these lines
                del images, labels ,features,outputs
                torch.cuda.empty_cache()
                gc.collect()
                time.sleep(2)
...
group1 = np.arange(50, 100, 1)
group2 = np.arange(0, 50, 1)
...
for iter in range(global_epochs):
    ...
    for idx in idxs_users:
        ...
        #added these lines
        to_be_copied= copy.deepcopy(net_glob_client).to(device)
        # Training ------------------
        w_client = local.train(net = to_be_copied)
        ...
        #added these lines
        local.evaluate(net = to_be_copied, ell= iter)
        del to_be_copied,w_client
        torch.cuda.empty_cache()
        gc.collect()
