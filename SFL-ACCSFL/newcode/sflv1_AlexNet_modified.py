...
import copy
from models import AlexNet_client_side
from models import AlexNet_server_side
...
program = "SFLV1_alexnet_cut3_modified_full"
print(f"---------{program}----------")
start = time.time()
...
def evaluate_full_model_all_clients(net_glob_client, net_model_server, clients, device, round_num):
    ...
    file_name = "full_model_accuracy_cut_layer_3.xlsx"
   # round=round+1
    new_data = {
            "round": 0,
...
def evaluate_full_model(net_client, net_server, idx, test_loader, ell):
    global batch_acc_test, batch_loss_test
    global loss_test_collect_user, acc_test_collect_user
    global acc_test_collect, loss_test_collect
    global count2, num_users
    ...
    if fed_check:
        ...
        print("==========================================================")
        # --- Write one new line to Excel per round ---
        file_name = "full_model_accuracy_cut_layer_4.xlsx"
        data_dict = {
            "round": [ell],
            "acc_train": [acc_avg_all_user_train],
            "acc_test": [acc_avg_all_user]
        }
        ...
        if not os.path.isfile(file_name):
            ...
            with pd.ExcelWriter(file_name, mode='a', engine='openpyxl', if_sheet_exists='overlay') as writer:
                # Append below existing data starting at next empty row
                # openpyxl engine with mode='a' appends to sheet but to append rows properly,
                # we use startrow param:
...
class Client(object):
    def __init__(self, net_client_model, idx, lr, device, dataset_train = None, dataset_test = None, idxs = None, idxs_test = None):
        ...
        self.device = device
        self.lr = lr
        self.local_ep = 1#set the number of local epochs before aggregation
    ...
    def train(self, net):
        net.train() #it sets the mode to 'train' so the layers are updated. If mode is 'eval', then the layers are frozen.
        ...
        for iter in range(self.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.ldr_train):
                ...
                #---------forward prop-------------
                fx = net(images) #net fuction creates a neural net
...
for iter in range(global_epochs):
    ...
    print("-----------------------------------------------------------")
    clients_eval = [
    Client(net_client_model=net_glob_client,      idx=i,      lr=lr,device=device,       dataset_train=dataset_train,dataset_test=dataset_test,idxs=dict_users[i],idxs_test=dict_users_test[i]) for i in range(num_users)]

    evaluate_full_model_all_clients( net_glob_client=net_glob_client, net_model_server=net_model_server, clients=clients_eval, device=device, round_num=iter)
