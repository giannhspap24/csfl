...
lamda1=0.02 #percentage of aggregators
lamda2=0.1 #percentage of aggregators
lamda3=0.3 #percentage of aggregators
v=4
h=1
h1=2
...
comm_per_round_sfl=(((layer_mbytes_activations[h]*B*2 + 2* sum(layer_mbytes_parameters[:h])) * (lamda1*N)) + (layer_mbytes_activations[v]*B *N) + (2* sum(layer_mbytes_parameters[h:v])) ) / 1000 #convert MB to GB
comm_per_round_sfl_tirana=(((layer_mbytes_activations[h]*B*2 + 2* sum(layer_mbytes_parameters[:h])) * (lamda2*N)) + (layer_mbytes_activations[v]*B *N) + (2* sum(layer_mbytes_parameters[h:v])) ) / 1000 #convert MB to GB
#comm_per_round_accsfl=((1*layer_mbytes_activations[v]*B + 2 * sum(layer_mbytes_parameters[:v])) * N ) / 1000 #convert MB to GB
comm_per_round_csfl=(((layer_mbytes_activations[h1]*B*2 + 2* sum(layer_mbytes_parameters[:h1])) * (lamda3*N)) + (layer_mbytes_activations[v]*B *N) + (2* sum(layer_mbytes_parameters[h1:v])) ) / 1000 #convert MB to GB
...
plt.plot(comm_csfl, full_accuracies_smoothed1,linestyle='--', color='c', label='HSFL(λ=0.05)', linewidth=4, markersize=1)
plt.plot(comm_sfl_tirana, full_accuracies_smoothed2,linestyle='-', color='g', label='HSFL(λ=0.15)', linewidth=4, markersize=1)
plt.plot(comm_sfl, full_accuracies_smoothed3, linestyle='-', color='b', label='HSFL(λ=0.3)', linewidth=4, markersize=1)
#plt.plot(comm_accsfl, full_accuracies_smoothed4, linestyle='-', color='m', label='ACCSFL(CONV-5)', linewidth=4, markersize=1)
