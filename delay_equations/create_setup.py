...
def assign_layers_iterative(devices, layer_mflops, v_range=(2, 5)):
    num_layers = len(layer_mflops)
...
if __name__ == "__main__":
    ...
    last_layers = 3
    #assignment_matrix, device_ids = assign_layers(devices, cpu_min=12.0, cpu_max=44.8)
    layer_assignments = assign_layers_tirana(devices, layer_mflops_one_sample)

    # Output assignments
    for dev_id, layers in layer_assignments.items():
        print(f"{dev_id}: {layers}")
        print("\nStrong Device Workloads (MFLOPs from weak clients):")
   
    delays = compute_delays_clients(devices, layer_assignments, layer_mflops_one_sample, samples)
    delay_server=compute_delay_server(num_devices,layer_mflops_one_sample, samples,v,V,p_s)
    delay_client=max(delays)
    delay_download=compute_delay_download(num_devices,layer_mbytes,v,V,R)
    # Show result
    slowest_client_delay=0
    for dev_id, delay in delays.items():
        print(f"{dev_id}: {delay:.2f}  (seconds)")
        if slowest_client_delay<delay:
            slowest_client_delay=delay

    print("delay slowest client:"+ str(slowest_client_delay) +" (seconds)")
    print("delay server:"+ str(delay_server) +" (seconds)")
    print("delay download:"+ str(delay_download) +" (seconds)")
    delay_accSFL=delay_download+(slowest_client_delay/3)+max(delay_server, (2*slowest_client_delay/3) )+delay_download
    print("ACCSFL total delay:"+ str(delay_accSFL) +" (seconds)")
    ...
    print("delay slowest client with my solution:"+ str(slowest_client_delay) +" (seconds)")

       
    #delays = compute_delays_clients(devices, assignments, layer_mflops_one_sample, samples)
    #delay_server=compute_delay_server(num_devices,layer_mflops_one_sample, samples,v,V,p_s)
    #delay_client=max(delays)
    #delay_download=compute_delay_download(num_devices,layer_mbytes,v,V,R)
    # Show result
